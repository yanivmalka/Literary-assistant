    ...
    <div>
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => setSelectedVersion('main')} className="text-start">
            <Card className={`p-6 transition-all ${selectedVersion === 'main' ? 'border-primary bg-primary-soft/40' : 'hover:border-muted-foreground/50'}`}>
              ...
              <div className="font-display text-2xl font-bold text-primary">{mainLocations.length}</div>
              <p className="text-xs text-muted-foreground mt-1">{t('entities.types.location')}</p>
            </Card>
          </button>
          {currentBranch ? (
            <button onClick={() => setSelectedVersion('branch')} className="text-start">
              <Card className={`p-6 transition-all ${selectedVersion === 'branch' ? 'border-primary bg-primary-soft/40' : 'hover:border-muted-foreground/50'}`}>
                <div className="flex items-center gap-2 mb-2"><GitBranch className="h-4 w-4" /><div className="font-display font-semibold text-lg">{currentBranch.name}</div></div>
                <p className="text-sm text-muted-foreground mb-4">{t('branch.branchDescription')}</p>
                <div className="font-display text-2xl font-bold text-primary">{branchLocations.length}</div>
                <p className="text-xs text-muted-foreground mt-1">{t('entities.types.location')}</p>
              </Card>
            </button>
          ) : <div className="border-2 border-dashed border-border rounded-lg p-6 opacity-50 cursor-not-allowed"><div className="flex items-center gap-2 mb-2"><GitBranch className="h-4 w-4" /><div className="font-display font-semibold text-lg">{t('branch.noBranch')}</div></div><p className="text-sm text-muted-foreground">{t('branch.createBranchFirst')}</p></div>}
        </div>
      </div>
